...
def _plain_title(title_html: str) -> str:
    """
    Plain-text rendering of a title for contexts that can't contain markup
    at all — <title>, and meta/attribute values like og:title. m["title"]
    may carry real tags (e.g. a bold Notion title becomes
    "<strong>...</strong>", not just escaped text), which is fine inside
    the <h1> but shows up as literal "<strong>" in a browser tab or link
    preview if used as-is. Strips those tags, decodes the &/</> entities
    _rich_text_to_html applied to the original text, then re-escapes for
    safe use in an HTML text node or attribute.
    """
...
# ── Social icons (self-contained, no external CDN — mirrors profile_index.py) ─
...
_SOCIAL_SVGS = {"twitter": _TWITTER_SVG, "github": _GITHUB_SVG, "linkedin": _LINKEDIN_SVG}
_SOCIAL_LABELS = {"twitter": "X (Twitter)", "github": "GitHub", "linkedin": "LinkedIn"}


def _build_social_sidebar(social_links: dict | None) -> str:
    """
    Floating column of the author's social links, mirroring the TOC sidebar
    on the opposite side. Renders nothing if the author hasn't set any.
    """
    if not social_links:
        return ""
    items = []
    for key in ("twitter", "github", "linkedin"):
        url = social_links.get(key)
        if not url:
            continue
        items.append(
            f'<a class="social-link" href="{_html_escape(url)}" '
            f'target="_blank" rel="noopener noreferrer" aria-label="{_SOCIAL_LABELS[key]}">'
            f"{_SOCIAL_SVGS[key]}</a>"
        )
    if not items:
        return ""
    return f'<aside class="social-sidebar" aria-label="Author social links">{"".join(items)}</aside>'
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    /* Author's social links — mirrors the TOC sidebar on the left, in the
       grid's first column. A vertical icon stack needs far less horizontal
       room than the TOC's text list, so it shows up at a smaller breakpoint
       — readers on medium screens get the icons even before there's room
       for the TOC too. */
    .social-sidebar {{
      display: none;
      grid-column: 1;
    ...
    .promo-card a:hover {{ text-decoration: underline; }}
...
<body>
  ...
  <article>
    ...
    <div class="promo-card">
      <span>💬 Enjoyed this? <a href="https://app.notohub.com/signup">Start your own blog on NotoHub →</a></span>
    </div>
...
def _render_html(
    ...
    plain_title = _plain_title(m["title"])

    toc_entries = _extract_toc(blocks)
    heading_anchors = {e["notion_id"]: e["anchor"] for e in toc_entries}

    cover_html = (
        f'<img class="cover-image" src="{cover_image_url}" alt="{plain_title}">'
        if cover_image_url else ""
    )
    social_sidebar_html = _build_social_sidebar(social_links)
    ...
    og_tags = [
        ...
        f'<meta property="og:title" content="{plain_title}">',
    ]
    if description:
    ...
    og_tags.append(f'<meta name="twitter:title" content="{plain_title}">')
    og_meta_html = "\n  ".join(og_tags)
    ...
    return _HTML_TEMPLATE.format(
        ...
        read_time=read_time,
        body=body,
        social_sidebar_html=social_sidebar_html,
...
def render_page_data(
    ...
    Args:
        ...
        social_links:      Author's {twitter, github, linkedin} URLs, if set — rendered
                           as a floating icon column beside the article. Omitted entirely
                           when the author hasn't set any.
    """
    return _render_html(
        page_data,
        author=author,
