...
notion_to_html.py
-----------------
Converts a Notion page URL into a Medium-style index.html file.

Auth is intentionally decoupled — swap `token_auth()` for your own
OAuth / service-account function and pass it into `notion_to_html()`.
"""
...
def token_auth(token: str) -> Callable[[], dict]:
    ...
    Usage:
        auth = token_auth("secret_abc123...")
        notion_to_html(url, auth)

    To swap in OAuth / client-id flow in production, write a function with
    the same signature:
    ...
    and pass `my_oauth_auth` (not called) to `notion_to_html()`.
    """
...
def _extract_page_id(url: str) -> str:
    ...
    # Last path segment
    slug = clean.split("/")[-1]
    # Remove title prefix separated by last '-'
    raw = re.sub(r"^.*-([a-f0-9]{32})$", r"\1", slug, flags=re.IGNORECASE)
    # Accept plain UUIDs too (with dashes)
    raw = raw.replace("-", "")
...
def _fetch_blocks(block_id: str, auth_fn: Callable[[], dict]) -> list:
    """
    Recursively fetches all block children for a given block/page ID,
    attaching nested children directly onto each block as block["_children"].
    ...
    # Recurse into any block that has children
    for block in blocks:
...
def fetch_notion_page(page_id: str, auth_fn: Callable[[], dict]) -> dict:
    ...
    Returns {"meta": {...}, "blocks": [...]}
    Author is NOT fetched here — pass it explicitly to notion_to_html().
    """
...
def _list_item_to_html(block: dict, tag: str, depth: int) -> str:
    """
    Renders a single bulleted/numbered list item (<li>) including any
    nested children as an indented sub-list.
    """
    ...
    inner = html_text
    if children:
        # Recurse: render children as a nested list, indented
...
def _block_to_html(block: dict, depth: int = 0) -> str:
    ...
    if btype == "paragraph":
        inner = html_text or "&nbsp;"
        # Paragraphs can have children in Notion (indented text blocks)
        if children:
            inner += "\n" + _blocks_to_html(children, depth=depth + 1)
        ...
        # Handled by _blocks_to_html — should not be called directly at top level
    ...
    # Unsupported — render as comment so nothing is silently lost
    return f"<!-- unsupported block type: {btype} -->\n"


def _blocks_to_html(blocks: list, depth: int = 0) -> str:
    """
    Converts a list of Notion blocks (with pre-fetched _children) into HTML.
    Consecutive bulleted/numbered list items are grouped into <ul>/<ol>.
    Sublists are handled recursively via each item's _children.
    """
    lines = []
    indent = "  " * depth
    i = 0
...
def _extract_meta(meta: dict) -> dict:
    ...
    # Title: Notion stores it under a "title"-type property
    title = ""
...
def _estimate_read_time(blocks: list) -> int:
    """
    Counts all words across the block tree (including nested children)
    and returns estimated reading minutes at 238 wpm (average adult rate).
    """
    def _count_words(blocks: list) -> int:
        ...
        for b in blocks:
            ...
            # Recurse into nested children (sublists, toggles, etc.)
            total += _count_words(b.get("_children", []))
        return total

    words = _count_words(blocks)
  ...
  <style>
    ...
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    :root {{
      ...
      --bg:         #ffffff;
      --bg-subtle:  #f9f9f9;
      --accent:     #1a8917;     /* Medium-style green */
      --border:     #e6e6e6;
      --max-w:      680px;
    }}
    ...
    .avatar {{
      ...
      object-fit: cover; /* also applies when avatar is an <img> */
    }}
...
<body>
  ...
  <nav class="top-bar">
    <a class="nav-logo" href="#">Medium</a>
...
def _render_html(page_data: dict, author: str) -> str:
    """Assembles the final HTML string from page data."""
    ...
    # Cover image removed (not supported)
    cover_html = ""
    ...
    # Author avatar: coloured initial from supplied name
    ...
    # Last-edited from Notion metadata (shown in footer only)
...
def notion_to_html(
    ...
    """
    Main function. Converts a Notion page URL to a styled index.html.

    Args:
        url:         Full Notion page URL.
        auth_fn:     A zero-argument callable that returns HTTP headers dict.
                     Use `token_auth("secret_...")` for token-based auth,
                     or plug in your own OAuth factory.
        author:      Author display name shown in the byline.

    Returns:
        The output file path.

    Example:
        from notion_to_html import notion_to_html, token_auth
    ...
    html      = _render_html(page_data, author=author)

    output_path = os.path.join(os.path.dirname(__file__), "output", "index.html")
    os.makedirs(os.path.dirname(output_path), exist_ok=True)  # creates ./output/ if missing
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python notion_to_html.py <notion-url> <token> [author] [output.html]")
        sys.exit(1)

    notion_url  = sys.argv[1]
    token       = sys.argv[2]
    author_arg  = sys.argv[3] if len(sys.argv) > 3 else "Unknown Author"

    auth = token_auth(token)
    notion_to_html(notion_url, auth, author=author_arg)